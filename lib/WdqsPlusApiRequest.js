...
module.exports = class WdqsPlusApiRequest {
	 ...
	 * @param {Object.<string, string>} extraResponseHeaders
	 * Additional headers to add to the response.
	 */
	constructor( query, transform, extraResponseHeaders = {} ) {
		this.query = query;
	...
	respond( request, response ) {
		...
		axios.get( url, { headers, responseType: 'stream' } ).then(
			...
			( wdqsError ) => {
				if ( wdqsError.response ) {
					response.writeHead( wdqsError.response.status, wdqsError.response.headers );
					wdqsError.response.data.pipe( response );
				} else {
