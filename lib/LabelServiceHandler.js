...
module.exports = class LabelServiceHandler {
	...
	handle( query, parsedQuery ) {
		...
		if ( languages.length === 1 ||
			...
			requestClass = 'EntityTermsRequest';
			...
			requestClass = 'WbgetentitiesRequest';
		} else {
		...
		outer: for ( const termVariable of termVariables ) {
			for ( const [ suffix, termType ] of [
				...
				if ( allVariableNames.has( baseVariable ) ) {
					variableMappings[ termVariable ] = { baseVariable, termType };
		...
		const newQueryString = new SparqlGenerator( { prefixes: allPrefixes } ).stringify( newQuery );
